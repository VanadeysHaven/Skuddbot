...
public class EventSubscriberAdapter extends BaseSubscriber<Event> {
    ...
    private static final Logger log = Loggers.getLogger(EventSubscriberAdapter.class);
    ...
    public void onGuild(GuildEvent event) {

    }

    public void onGatewayLifecycle(GatewayLifecycleEvent event) {
    ...
    public void onRole(RoleEvent event) {

    }

    public Flux<Event> listener(Flux<Event> events) {
        return events.flatMap(event -> Mono.fromRunnable(() -> hookOnEvent(event))
                .onErrorResume(t -> {
    ...
    public final void hookOnEvent(Event event) {
        ...
        if (event instanceof ChannelEvent) onChannel((ChannelEvent) event);
        else if (event instanceof GuildEvent) onGuild((GuildEvent) event);
        else if (event instanceof GatewayLifecycleEvent) onGatewayLifecycle((GatewayLifecycleEvent) event);
        else if (event instanceof MessageEvent) onMessage((MessageEvent) event);
        else if (event instanceof RoleEvent) onRole((RoleEvent) event);
    }
    ...
    protected void hookOnError(Throwable t) {
        log.error("An error terminated this subscriber", t);
    }
}